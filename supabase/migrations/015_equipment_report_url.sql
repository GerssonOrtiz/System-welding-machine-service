-- 015_equipment_report_url.sql
-- Añade campo report_url a equipment_records para almacenar el link de informe (Google Drive PDF)
-- y actualiza la vista equipment_with_status para incluirlo

-- 1. Añadir columna report_url a equipment_records
ALTER TABLE public.equipment_records
ADD COLUMN IF NOT EXISTS report_url TEXT DEFAULT NULL;

-- 2. Recrear la vista equipment_with_status incluyendo report_url (ya viene en er.*)
-- La vista usa SELECT er.* por lo que report_url ya estará incluido automáticamente.
-- Recreamos de todas formas para ser explícitos y mantener coherencia con migraciones anteriores.
